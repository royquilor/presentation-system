...
function buildObservations(keyPoints, maxObs = 8) {
  const obs = [];
  const seen = new Set();
  ...
  if (obs.length === 0) {
    obs.push({ number: 1, title: "Document overview", body: "Key details are outlined in the source Confluence page." });
  }
...
function generatePresentation({
  ...
  md += `*Aligned with the broader FSD programme strategy and ServiceNow best practices.*\n\n---\n\n`;
