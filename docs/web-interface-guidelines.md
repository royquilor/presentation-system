...
- **MUST** — non‑negotiable for accessibility, correctness, or safety.
- **SHOULD** — recommended unless there’s a good reason not to.
...
- MUST: External links with `target="_blank"` use `rel="noopener noreferrer"`
...
- MUST: Use polite `aria-live` for toasts, inline validation, and loading/processing states
...
- MUST: If it looks clickable, it must be clickable and keyboard-activatable
...
- MUST: Animations interruptible and input-driven (no autoplay); respect `prefers-reduced-motion` (see reduced variant above)
...
- SHOULD: Curly quotes (" "); avoid widows/orphans (`text-wrap: balance` or Tailwind `text-balance`)
...
- MUST: Sanitize user-generated HTML before rendering (XSS); never expose tokens in URLs
